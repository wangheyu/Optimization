...
\end{equation}
$$
其中$[Q_1 \quad Q_2]$是正交矩阵，$Q_1$和$Q_2$分别是$n \times m$和$n \times (n - m)$的正交块，而$R$是$m \times m$的上三角块。$m \times m$阵$\Pi$是交换矩阵（具体的QR分解算法见任何一本数值代数教材，或者本书附录(A.24)）。现在，可以令$Y = Q_1$，$Z = Q_2$。这样$Y$和$Z$的列向量一起构成了$\mathbb{R}^n$空间的一组正交基。由(15.21)，有
$$
AY = \Pi R^T, \quad AZ = 0.
$$
